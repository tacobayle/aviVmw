...
output "destroy" {
  value = "ssh -o StrictHostKeyChecking=no -i ~/.ssh/${basename(var.jump["private_key_path"])} -t ubuntu@${vsphere_virtual_machine.jump.default_ip_address} 'git clone ${var.jump["ansibleAviPbAbsent"]}; ansible-playbook ansiblePbAviAbsent/local.yml --extra-vars @~/ansible/vars/fromTerraform.yml --extra-vars @~/ansible/vars/creds.json' ; sleep 20 ; terraform destroy -auto-approve"
